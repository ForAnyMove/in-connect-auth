...
serve(async (req) => {
  ...
  try {
    ...
    // Создание SyncVK данных
    const syncvkRes = await fetch(`https://panel.syncvk.com/api/users`, {
      method: 'POST',
      headers: {
    ...
    if (updateError) {
      console.error('Ошибка при создания ключа:', updateError);
      return jsonResponse({ error: 'Не удалось создать ключ' }, 500);
    }
